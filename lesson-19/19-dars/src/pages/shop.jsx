...
function Shop({ products, selectedGenres, setSelectedGenres, wishlist, setWishlist }) {
  ...
  useEffect(() => {
    let newProducts = products.filter(
      (product) =>
        (product.discountedPrice >= range[0] || product.originalPrice >= range[0]) &&
        (product.discountedPrice <= range[1] || product.originalPrice <= range[1])
    );
    if (selectedGenres.length) {
      newProducts = newProducts.filter(
        (pr) => selectedGenres.findIndex((gr) => gr.title.toUpperCase() == pr.genre.toUpperCase()) !== -1
      );
    }
    ...
    <div>
      ...
      <div className='flex justify-around mt-8 gap-5 flex-wrap'>
        {filteredProducts?.length ? (
          ...
          <div className='text-center text-[48px]'>No filtered Data</div>
        )}
      </div>
    </div>
  );
